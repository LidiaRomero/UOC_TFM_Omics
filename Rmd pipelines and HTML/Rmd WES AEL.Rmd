---
title: "Rmd for variant analysis from a VCF file: WES from patient with acute erythroid leukemia"
...
First we read the data from the VCF file using the readVcf function of Bioconductor's VariantAnnotation package. To ease the processing of the information, we will keep the data from only one chromosome (9) since the intention is to set up the R pipeline to perform this analysis.

```{r}
vcf <- readVcf("m_4_D_single_sample.vcf", "hg19")
vcf_chr <- vcf[grepl(names(vcf), pattern = "chr9:")]
```
...
matS$frequency <- allele_data[[sample_name]][match(matS$Variant, rownames(allele_data))]
matS$frequency <- as.character(matS$frequency)
...
## Anàlisi de sobrerepresentació mitjançant GO
...
## Anàlisi d'enriquiment per vies KEGG
...
## Anàlisi d'enriquiment basat en vies REACTOME
...
We see how the variants are distributed along chromosome 9.

```{r, fig.cap="Figura 4. Variants al llarg del cromosoma 9."}
...
Finally, we look at the results of the analyzes of biological significance.

```{r, fig.cap= "Figura 5. Visualització del resultat d'enriquiment funcional. A: dotplot del resultat d'enriquiment GO. B: representació gràfica de la ontologia gènica en l'anàlisi d'enriquiment. C i D: Linkage entre gens i termes GO."}
